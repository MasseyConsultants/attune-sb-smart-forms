...
// Purpose: Proxies email verification tokens to the API.
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    apiRes = await fetch(`${getApiUrl()}/auth/verify-email`, {
      method: 'POST',
    ...
    console.error('[verify-email] upstream API unreachable:', err);
    return NextResponse.json(
      {
        success: false,
